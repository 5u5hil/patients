...
function addOther(name, field, val) {
    if (jQuery('#' + name + '-1').length == 0) {
        if (val == 0) {
            var htmlStr = '';
            htmlStr = "<div id='" + name + "-1'><label class='item item-input item-floating-label'><input type='text' name='new-" + name + " ng-model='record." + name + "' placeholder='Please Enter " + field + "' required /></label></div>";
